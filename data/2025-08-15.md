...
TL;DR: 提出了一种自动生成量子比特映射和路由（QMR）编译器的方法，适用于任意量子架构，通过领域特定语言Marol简化问题定义，并验证其性能与手工编译器相当。


<details>
  <summary>Details</summary>
Motivation: 量子计算机的潜力受限于多样化的硬件架构和快速变化的需求，需要一种通用方法简化QMR编译器的开发。

Method: 基于设备状态机的核心结构，开发了领域特定语言Marol，用于定义QMR问题，并通过参数化求解器解决。

Result: 生成的编译器在运行时和解质量上与手工编译器相当，适用于多种硬件平台和量子架构。

Conclusion: 该方法为未来量子编译器开发提供了简化途径，适应不断涌现的新架构。
...
Method: 提出Saracoder框架，包括分层特征优化模块（语义关系提炼、重复修剪、基于图的结构相似性评估）和外部感知标识符消歧模块。
