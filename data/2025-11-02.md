...
TL;DR: 论文展示了如何通过有界工作实现公平性，这一思想不仅适用于miniKanren的搜索策略，也适用于最坏情况最优连接的实现。


<details>
  <summary>Details</summary>
Motivation: 探索如何在关系型查询处理中实现公平的搜索策略，并验证该思想在最坏情况最优连接中的适用性。

Method: 采用有界工作的方法来保证公平性，通过可搜索迭代器接口实现组合式的最坏情况最优连接。

Result: 成功证明了公平性通过有界工作的思想可以优雅地应用于最坏情况最优连接的实现。

Conclusion: 有界工作的公平性概念是连接miniKanren搜索策略和最坏情况最优连接的通用原则，适合在函数式语言中浅层嵌入。
...
TL;DR: CHCVERIF是一个基于组合策略的CHC求解器，采用软件验证方法来解决约束Horn子句问题，能够复用成熟的软件验证工具处理涉及位向量和低级语义的基准测试。


<details>
  <summary>Details</summary>
Motivation: 约束Horn子句（CHCs）被广泛用作各种验证任务的中间表示，包括安全检查、不变式合成和过程间分析。本文旨在探索使用软件验证工具作为CHC求解后端的可行性和潜力。

Method: 开发了CHCVERIF，一个基于组合策略的CHC求解器，采用软件验证方法来解决CHCs，能够复用成熟的软件验证工具处理CHC基准测试，特别是涉及位向量和低级语义的问题。

Result: 评估显示该方法在线性整数算术上仅取得中等成功，但在位向量基准测试上取得了适度成功。结果证明了使用软件验证工具作为CHC求解后端的可行性和潜力。

Conclusion: 使用软件验证工具作为CHC求解后端是可行且有潜力的，特别是在精心构建的组合策略支持下，能够有效处理涉及位向量和低级语义的CHC问题。
...
### [3] [SymCode: A Neurosymbolic Approach to Mathematical Reasoning via Verifiable Code Generation](https://arxiv.org/abs/2510.25975)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂数学推理中表现不佳，基于文本的生成导致未经验证和算术不健全的解决方案，现有提示策略缺乏确定性验证机制。

Method: 引入SymCode框架，将数学问题解决重新定义为使用SymPy库的可验证代码生成任务，将LLM推理建立在确定性符号引擎上。

Result: 在MATH-500和OlympiadBench等挑战性基准测试中，比基线准确率提高了13.6个百分点，不仅更节省token，还将模型失败从模糊的逻辑谬误转向透明的程序错误。
...
### [4] [QCoder Benchmark: Bridging Language Generation and Quantum Hardware through Simulator-Based Feedback](https://arxiv.org/abs/2510.26101)
...
TL;DR: 提出了QCoder Benchmark评估框架，用于评估LLM在量子编程任务上的表现，支持量子模拟器环境和真实人类代码对比分析。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在需要与硬件设备交互的领域（如量子编程）表现不足，缺乏专门的评估框架。

Method: 构建包含量子模拟器环境的评估框架，支持电路深度、执行时间等域特定指标反馈，并整合真实编程竞赛中的人类代码提交。

Result: 实验显示GPT-4o仅18.97%准确率，而推理模型o3达到78%准确率，超过人类代码平均成功率（39.98%）。

Conclusion: QCoder Benchmark填补了量子编程评估的空白，揭示了LLM在该领域的挑战，并提供了公开数据集和API支持后续研究。
...
### [5] [Finding Regular Herbrand Models for CHCs using Answer Set Programming](https://arxiv.org/abs/2510.26428)
*Gregoire Maire,Thomas Genet*

Main category: cs.LO

TL;DR: 提出了一种使用Clingo和ASP将带ADT的CHC编码为SAT问题的方法，用于构建树自动机识别CHC的Herbrand模型，实现半完备的可满足性检查器。
...
Method: 将带ADT的CHC编码为ASP问题，使用Clingo工具求解，从而构建树自动机。

Result: 实现了一个半完备的可满足性检查器：当存在正则Herbrand模型时找到树自动机，当问题不可满足时找到反例。

Conclusion: 该方法为带ADT的CHC可满足性验证提供了一种有效的ASP编码解决方案。
...
### [6] [Semantic Properties of Computations Defined by Elementary Inference Systems](https://arxiv.org/abs/2510.26429)
*Salvador Lucas*

Main category: cs.LO

TL;DR: 该论文提出使用基本推理系统定义集合/关系/计算，通过一阶理论Th(I)和规范模型来验证语义属性，并展示如何在任意模型中证明语义属性，应用于编程语言和重写系统的分析。


<details>
  <summary>Details</summary>
Motivation: 研究如何通过基本推理系统描述编程语言和系统的计算，并验证其语义属性，特别是针对重写系统等计算模型。

Method: 使用Smullyan的基本形式系统和Gentzen的推理规则表示法构建基本推理系统I，通过一阶理论Th(I)和规范模型验证语义属性，并扩展到任意模型中的可满足性验证。

Result: 提出了在任意模型而非仅规范模型中验证语义属性的方法，使得对基本推理系统定义的集合/关系/计算的性质分析更加灵活和实用。

Conclusion: 该方法为分析编程语言和重写系统等计算模型的语义属性提供了有效的理论框架，特别是在规范模型不可计算的情况下，通过任意模型验证语义属性具有重要应用价值。
