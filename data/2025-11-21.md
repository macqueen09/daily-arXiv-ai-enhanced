...
Motivation: 大多数依赖类型语言通过归纳类型很好地支持了第一种扩展性（添加新操作），但通过余归纳类型支持第二种扩展性（添加新构造函数）通常较差。Polarity语言虽然对称处理两种类型，但缺乏隐式参数等现代依赖类型语言应有的功能。

Method: 提供了Polarity类型系统的完整算法描述，以及覆盖任意归纳和余归纳类型的统一算法。给出了归约语义、转换检查和模式匹配统一算法的规则。

Result: 开发了算法的进行中实现，可在https://polarity-lang.github.io/获取。统一算法和设计决策可作为其他对称支持归纳和余归纳类型的依赖类型语言的蓝图。

Conclusion: 本文通过提供支持隐式参数的算法类型系统和统一算法，完善了Polarity语言的功能，使其能够更好地对称处理归纳和余归纳类型，为其他类似语言提供了参考。
...
TL;DR: Chorex是一个将编排编程引入Elixir的语言，支持容错的分布式应用开发，通过元编程实现与宿主语言的紧密集成。


<details>
  <summary>Details</summary>
Motivation: 为构建鲁棒的分布式应用程序，需要一种能够容忍参与者故障的编排编程语言。

Method: 使用元编程在Elixir中实现完整的编排功能，当参与者崩溃时自动生成新进程、恢复检查点状态并更新网络配置。

Result: Chorex成功实现了容错编排，能够静态检测编排需求与参与者实现的不匹配，并在多个示例中验证了其可行性。

Conclusion: Chorex的投影策略（输出无状态函数集）是支持可重启参与者的可行方法，可推广到其他语言。
...
<details>
  <summary>Details</summary>
Motivation: 微控制器虚拟机由于内存限制只能提供有限功能，现有虚拟机缺乏交互响应性和高执行速度，需要一种方法在保持微控制器优势的同时提供更丰富的功能

Method: 设计并实现了BlueScript虚拟机，将大部分组件卸载到主机上，使用影子机器数据结构来镜像微控制器的执行状态以减少通信开销

Result: 卸载组件不会严重影响预期收益，卸载的增量编译器比MicroPython和Espruino执行速度更快，同时保持与MicroPython相当的交互性，动态编译器提升了虚拟机性能
...
TL;DR: Operon是一个基于Rust的工作流引擎，通过命名维度和显式依赖关系的形式化方法，专门处理不规则数据（ragged data），在机器学习应用中实现高效的大规模数据生成流水线。


<details>
  <summary>Details</summary>
Motivation: 现代数据处理工作流经常遇到不规则数据，但现有工作流引擎缺乏对不规则数据形状和依赖关系的原生支持，用户需要手动管理复杂的索引和依赖关系。

Method: 开发了一个领域特定语言，用户通过维度注释声明流水线，系统静态验证正确性，运行时根据动态发现的数据形状增量调度任务。采用每任务多队列架构实现异构任务类型的高效并行。

Result: 经验评估显示Operon比现有工作流引擎性能提升14.94倍，在负载扩展时保持接近线性的端到端输出率。

Conclusion: Operon通过形式化部分已知状态的显式建模，为不规则数据处理提供了确定性、并发性和鲁棒性保证，特别适用于机器学习中的大规模数据生成流水线。
...
### [5] [CorrectHDL: Agentic HDL Design with LLMs Leveraging High-Level Synthesis as Reference](https://arxiv.org/abs/2511.16395)
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM在硬件前端设计中由于幻觉倾向导致的功能错误问题，同时结合LLM的生成能力和传统设计流程的严谨性。
...
### [6] [TB or Not TB: Coverage-Driven Direct Preference Optimization for Verilog Stimulus Generation](https://arxiv.org/abs/2511.15767)
...
Method: 使用覆盖率驱动的直接偏好优化(CD-DPO)微调LLM，引入PairaNet数据集（包含基于仿真覆盖率指标标注的高质量和低质量测试平台对），将定量覆盖率反馈直接集成到优化目标中。
