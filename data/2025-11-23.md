...
Motivation: Polarity语言虽然能对称处理归纳类型和余归纳类型，但缺乏隐式参数等现代依赖类型语言应有的功能，这限制了其表达能力。

Method: 开发了完整的算法类型系统描述和统一的推理算法，涵盖了任意归纳和余归纳类型，包括归约语义、转换检查和模式匹配的统一算法。

Result: 提供了Polarity类型系统的完整算法描述和统一的推理算法，并有一个正在进行中的实现版本。

Conclusion: 该工作为其他支持对称处理归纳和余归纳类型的依赖类型语言提供了统一算法和设计决策的蓝图。
...
TL;DR: Chorex是一个将编排编程引入Elixir的语言，通过元编程实现完整编排功能，支持参与者故障恢复，并通过检查点机制实现状态恢复和网络配置更新。


<details>
  <summary>Details</summary>
Motivation: 构建能够容忍参与者故障的分布式应用程序，实现编排编程与宿主语言的紧密集成。

Method: 使用元编程在Elixir中实现编排编程，通过检查点机制保存状态，当参与者崩溃时自动生成新进程并恢复状态，同时更新所有参与者的网络配置。

Result: 成功实现了支持故障恢复的编排语言Chorex，能够静态检测编排需求与参与者实现之间的不匹配，并在多个示例中验证了其有效性。

Conclusion: Chorex的输出无状态函数集的投影策略是支持可重启参与者的可行方法，可推广到其他语言。
...
<details>
  <summary>Details</summary>
Motivation: 微控制器虚拟机由于内存限制功能有限，现有虚拟机缺乏交互响应性和高执行速度，需要一种方法来利用主机的丰富内存和处理能力。

Method: 设计并实现了BlueScript VM，通过在主机上创建镜像微控制器执行状态的影子机器结构，将大部分组件卸载到主机，减少通信开销。

Result: 组件卸载不会严重影响预期效益，卸载的增量编译器比MicroPython和Espruino执行速度更快，同时保持与MicroPython相当的交互性，动态编译器提高了虚拟机性能。
...
<details>
  <summary>Details</summary>
Motivation: 现代数据处理工作流经常遇到不规则数据，现有工作流引擎缺乏对形状和依赖关系的原生支持，用户需要手动管理复杂的索引和依赖关系。

Method: 使用命名维度和显式依赖关系的形式化方法，提供领域特定语言进行静态验证，运行时系统根据增量发现的形状动态调度任务。

Result: Operon相比现有工作流引擎减少了14.94倍基线开销，在扩展工作负载时保持接近线性的端到端输出速率。

Conclusion: Operon特别适用于机器学习应用中的大规模数据生成管道，其部分已知状态建模实现了强大的持久化和恢复机制。
...
TL;DR: 提出TB or not TB框架，使用基于覆盖率驱动的直接偏好优化(CD-DPO)微调LLM来自动生成硬件验证激励，在CVDP CID12基准测试中比开源和商业基线表现更好，代码覆盖率提升达77.27%。


<details>
  <summary>Details</summary>
Motivation: 硬件设计验证是最耗时和资源密集的阶段，其中为被测设计生成有效激励既关键又劳动密集。随着LLM的快速发展，将其应用于硬件设计和验证具有重要价值。

Method: 提出TB or not TB框架，使用CD-DPO方法微调LLM，引入PairaNet数据集（源自PyraNet），该数据集使用仿真覆盖率指标标记高质量和低质量测试平台对。CD-DPO将定量覆盖率反馈直接集成到优化目标中，引导模型生成最大化验证覆盖率的激励。

Result: 在CVDP CID12基准测试中，TB or not TB优于开源和商业基线，代码覆盖率提升达77.27%。
...
TL;DR: 提出CorrectHDL框架，利用HLS结果作为功能参考来纠正LLM生成的HDL设计中的错误，在保持正确性的同时显著提升面积和功耗效率
