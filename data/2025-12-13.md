...
Method: 引入良基函子概念，在良基关系索引的族范畴上构建框架，证明良基函子的所有余代数都是递归的

Result: 主要理论结果：每个良基函子的余代数都是递归的；该框架能涵盖排名函数等传统技术；案例研究包括快速排序、欧几里得算法和CYK解析

Conclusion: 提出的框架使递归性成为类型保证的固有属性，简化了递归算法的形式化验证，已在Cubical Agda中实现
...
TL;DR: 论文提出了一种基于作用域效应的模块化抽象解释框架，通过将语法消除和领域语义引入处理器分离，实现灵活的前向/后向、过近似/欠近似分析。


<details>
  <summary>Details</summary>
Motivation: 现有抽象解释框架缺乏真正的灵活性，无法支持不同的路径敏感性和流敏感性分析，且语法与语义紧密耦合，实现缺乏模块化。当前模块化设计方法（如单子变换器）虽然提供模块性但过于复杂笨重。

Method: 提出累积抽象语义方法，利用作用域效应在解释器中累积语义片段，将效应分为两类：语法消除处理器和领域语义引入处理器，从而实现模块化设计。

Result: 展示了从单一解释器创建多个动态求值器和静态分析器的潜力，证明了效应作为设计工具能够构建清晰、优雅且模块化的抽象解释框架。

Conclusion: 使用作用域效应是实现模块化抽象解释框架的有效方法，能够分离语法和语义，支持灵活的分析配置，同时保持设计的简洁性和优雅性。
