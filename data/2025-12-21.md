...
TL;DR: LOOPRAG：基于检索增强生成的循环优化框架，通过参数驱动方法生成多样化合法示例，结合循环感知检索和反馈迭代机制，显著提升LLM在循环变换优化中的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管循环变换是广泛使用的语义保持优化技术，但找到最优变换组合仍然困难。现有LLM在循环优化中常产生错误或次优结果，错失性能提升机会。

Method: 提出LOOPRAG框架：1）参数驱动方法利用循环属性触发各种变换，生成多样化合法示例；2）基于循环特征的检索算法平衡相似性和多样性；3）反馈迭代机制结合编译、测试和性能结果指导LLM；4）通过变异、覆盖和差分测试进行等价性检查。

Result: 在PolyBench、TSVC和LORE基准测试中，相比基础编译器分别获得最高11.20×、14.34×和9.29×加速；相比基础LLM分别获得最高11.97×、5.61×和11.59×加速。

Conclusion: LOOPRAG有效解决了LLM在循环优化中的局限性，通过检索增强生成和反馈迭代机制显著提升了循环变换优化的效果，为代码优化提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 自主自适应系统在软件开发中面临重大挑战，特别是需要明确捕捉环境或决策过程中的不确定性。在安全和任务关键系统中，这些挑战更为突出，因为这些系统必须在设计和开发过程中经过严格审查。主要挑战之一是难以使用概率构造来指定需求以捕捉影响这些系统的不确定性。

Method: 扩展NASA正式需求获取工具(FRET)的结构化自然语言，支持概率需求的规范。开发一种形式化、组合式、自动化的方法，将结构化自然语言需求转换为概率时序逻辑公式。提供自动化验证框架和形式化证明来确保生成的公式符合预期语义。

Result: 扩展后的FRET工具使开发人员能够用结构化自然语言指定概率需求，并自动将其转换为概率时序逻辑，使自主自适应系统的形式化分析更加实用且减少错误。

Conclusion: 通过扩展FRET工具支持概率需求规范，并提供自动化转换和验证框架，显著提高了自主自适应系统形式化分析的实用性和可靠性，解决了开发人员直接使用复杂形式化方法的不现实性和易错性问题。
...
TL;DR: NeuroInv是一个神经符号方法，结合LLM和Hoare逻辑进行循环不变式生成，在150个Java程序上达到99.5%成功率


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的循环不变式生成方法缺乏可靠的结构化方法，且很少参考程序验证理论，需要更系统的方法

Method: 神经符号方法包含两个模块：1) 神经推理模块利用LLM和Hoare逻辑通过后向链最弱前置条件推理推导和精化候选不变式；2) 验证引导的符号模块使用OpenJML的反例迭代修复不变式

Result: 在150个Java程序基准测试中达到99.5%成功率，显著优于其他方法；在10个大型多循环程序(平均7个循环)的困难基准上也表现良好

Conclusion: NeuroInv展示了神经符号方法在循环不变式生成中的有效性，能够扩展到更复杂的验证场景
...
TL;DR: 该论文提出针对语言模型工具调用性能瓶颈的系统优化方案，通过推测工具调用和保持序列驻留来提升推理吞吐量，实现每秒数百token的性能提升。


<details>
  <summary>Details</summary>
Motivation: 语言模型越来越依赖外部工具（如文件搜索、代码执行、API调用等），这些工具虽然增强了模型能力，但也引入了推理过程中的性能瓶颈，影响了LM代理框架的效率。

Method: 提出两种系统优化：1) 推测工具调用，提前预测并执行可能的工具调用；2) 强制序列在推理引擎中保持驻留，减少开销。还建议新的"工具缓存"API端点。

Result: 优化方案使LM代理推理的吞吐量提升了每秒数百个token，显著改善了性能。提供了理论分析指导最优推测配置。

Conclusion: 通过系统层面的优化可以有效解决LM工具调用带来的性能瓶颈，提出的推测工具调用和序列驻留技术能显著提升推理效率，建议LM提供商采用工具缓存API。
