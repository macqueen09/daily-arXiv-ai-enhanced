...
TL;DR: AutoBaxBuilder是一个自动生成代码安全基准测试任务的框架，旨在解决手动创建基准测试的局限性，包括数据污染、任务扩展性和难度提升等问题。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在软件工程中的广泛应用，可靠评估LLM生成代码的正确性和安全性变得至关重要。现有手动创建的安全基准测试存在三个主要问题：1) 容易污染训练数据；2) 需要扩展到新任务以提供更全面的评估；3) 需要提高难度以挑战更强大的LLM。

Method: 提出AutoBaxBuilder框架，通过细粒度合理性检查的稳健流水线，利用LLM的代码理解能力构建功能测试和端到端安全探测利用。框架能够从零开始生成代码安全基准测试任务和测试。

Result: 使用AutoBaxBuilder构建了全新的AutoBaxBench基准测试集。实验表明，生成一个新任务的时间少于2小时，成本低于10美元。与人工专家构建的任务相比，该框架生成的基准测试质量得到验证。

Conclusion: AutoBaxBuilder能够高效、低成本地自动生成高质量的代码安全基准测试任务，解决了手动创建基准测试的局限性，为评估LLM代码安全能力提供了可扩展的解决方案。
