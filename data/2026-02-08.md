...
TL;DR: Doc2Spec：一个多智能体框架，利用LLM从自然语言规则自动推导规范语法，并基于该语法生成形式化规范，以提升API实现和使用的合规性验证。


<details>
  <summary>Details</summary>
Motivation: 确保API实现和使用符合自然语言编程规则对软件正确性、安全性和可靠性至关重要。形式化验证能提供强保证，但需要精确的规范，而手动编写这些规范既困难又昂贵。

Method: 提出Doc2Spec多智能体框架：1）使用LLM从自然语言规则自动推导规范语法；2）基于推导出的语法指导生成形式化规范。语法捕获关键领域知识，约束规范空间，并确保表示一致性。

Result: 在三种编程语言的七个基准测试上评估，Doc2Spec优于无语法推导的基线方法，并与使用手动构建语法的技术取得竞争性结果，证明了自动语法推导在形式化自然语言规则方面的有效性。

Conclusion: Doc2Spec通过自动从自然语言规则推导规范语法，有效解决了形式化验证中规范编写困难的问题，提高了生成规范的可靠性和质量，为API合规性验证提供了实用解决方案。
...
TL;DR: 该论文研究了Ahman和Pretnar提出的异步效应演算的归一化性质，证明了在移除一般递归后，该演算（包括顺序和并行部分）是强归一化的，并进一步证明了在引入受控的中断驱动递归行为后，顺序片段仍保持强归一化。


<details>
  <summary>Details</summary>
Motivation: Ahman和Pretnar的异步效应演算为代数计算效应提供了异步处理能力，能够建模抢占式多线程、可取消远程函数调用、多方应用等场景。然而，该演算的终止性质尚未得到充分研究，需要分析其归一化性质以确保程序行为的可预测性。

Method: 采用Lindley和Stark基于⊤⊤-提升的方法来证明强归一化性质。首先从原始演算中移除一般递归，证明剩余演算（包括顺序和并行部分）是强归一化的。然后，在顺序片段中重新引入受控的中断驱动递归行为，证明其仍保持强归一化。所有证明均在Agda中形式化验证。

Result: 1. 移除一般递归后，异步效应演算的完整版本（包括顺序和并行部分）是强归一化的；2. 在顺序片段中引入受控的中断驱动递归行为后，该片段仍保持强归一化；3. 所有结果均在Agda中形式化验证，确保了证明的正确性。

Conclusion: 该研究为异步效应演算的终止性质提供了理论基础，证明了在适当限制递归行为的情况下，异步效应演算可以保证强归一化。这为异步编程语言的设计和实现提供了理论保障，特别是对于需要保证程序终止性的应用场景。
...
TL;DR: 使用代数效应理论为动态线程提供完整的等式公理化，基于参数化代数理论，通过fork和wait原语建模并发，证明模型完备性和语法完备性。


<details>
  <summary>Details</summary>
Motivation: 为动态线程提供完整的等式公理化，使用代数效应理论来形式化并发编程中的fork和wait操作，建立可靠的数学模型。

Method: 基于参数化代数理论，构建包含fork和wait原语的代数理论，提供操作语义和指称语义，证明完备性。

Result: 实现了对动态线程的完整等式公理化：模型完备性（闭表达式完全捕获标记偏序集相等性）和语法完备性（开表达式在所有闭化替换下相等时可证明相等）。

Conclusion: 代数效应理论为并发编程的形式化分析提供了有效框架，参数化代数理论能够简洁地建模线程创建和等待操作，并建立可靠的语义基础。
