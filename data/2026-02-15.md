...
TL;DR: GPT-5在低资源函数式编程语言Idris中通过基于编译器错误的迭代提示显著提升性能，从22/56提升到54/56
...
Method: 使用Exercism平台的Idris练习，首先建立零样本提示基线，然后评估多种精炼策略：基于平台反馈的迭代提示、增加文档和错误分类指南、使用本地编译错误和失败测试用例的迭代提示

Result: 零样本提示下GPT-5仅解决22/56个Idris问题，远低于Python（45/50）和Erlang（35/47）。使用本地编译错误的迭代提示方法效果最好，将性能提升到54/56个问题

Conclusion: 大型语言模型在低资源环境中可能最初表现不佳，但结构化的编译器级别反馈在解锁其能力方面起着关键作用
...
TL;DR: Yaksha-Prashna是一个系统，允许操作员/开发者验证eBPF字节码是否符合其规范并检查与其他字节码的依赖关系，提供200-1000倍的性能提升。


<details>
  <summary>Details</summary>
Motivation: 云基础设施组织越来越多地依赖第三方eBPF网络功能，但这些功能以字节码形式提供，操作员无法理解其功能正确性和与其他网络功能的交互。同时，eBPF开发者希望在不公开源代码的情况下证明其网络功能的正确性。

Method: 设计Yaksha-Prashna系统，构建领域特定模型，采用可扩展的程序分析技术来提取和建模eBPF程序。使用Yaksha-Prashna语言表达属性验证。

Result: 在标准和非常规eBPF网络功能上表达了24个属性，相比最先进的工作实现了200-1000倍的加速。

Conclusion: Yaksha-Prashna系统有效解决了第三方eBPF网络功能验证问题，为操作员和开发者提供了强大的验证工具，显著提升了验证效率。
