...
TL;DR: AutoVeriFix+：一个三阶段框架，通过结合高层语义推理和状态空间探索，提升LLM生成Verilog代码的功能正确性和设计效率


<details>
  <summary>Details</summary>
Motivation: LLM在生成高级编程语言代码方面表现出色，但在硬件描述语言（如Verilog）上应用困难，主要因为高质量训练数据稀缺。现有方法多关注语法正确性，导致生成代码存在功能错误。

Method: 三阶段框架：1) LLM生成高层Python参考模型定义电路行为；2) LLM生成初始Verilog RTL候选并迭代修复语法错误；3) 引入Concolic测试引擎执行深度顺序逻辑并识别边界情况漏洞，提供周期精确执行轨迹和内部寄存器快照，帮助LLM解决复杂状态转换错误，同时生成覆盖率报告进行语义剪枝以优化面积。

Result: 在严格基准测试中实现超过80%的功能正确性，在VerilogEval-machine数据集上达到90.2%的pass@10分数。通过轨迹感知优化，在基准测试中平均消除25%的冗余逻辑。

Conclusion: AutoVeriFix+通过结合高层语义推理和状态空间探索，显著提升了LLM生成Verilog代码的功能正确性和设计效率，解决了现有方法在硬件描述语言生成中的局限性。
...
TL;DR: 本文提出了一种类型安全的方法，将参数化流集成到流式监控框架RTLola中，以确保监控器在处理无界数据域时的运行时安全性。


<details>
  <summary>Details</summary>
Motivation: 流式监控是无人机等复杂信息物理系统的实时安全保障机制。由于监控器是安全关键组件，必须确保其运行时无错误。当监控器需要处理无界数据域（如无限数量的空域参与者）时，这一挑战尤为严峻，需要使用动态数据结构。

Method: 提出参数化流的概念，将单个流推广到无界数量流实例的集合，并提供系统化的内存管理机制。采用精化类型系统来保证所有内存引用要么成功，要么有默认值支持，从而确保运行时错误的不存在。

Result: 证明了运行时错误的缺失在一般情况下是不可判定的，但可以通过精化类型系统有效确保。在包括自主飞机监控在内的多个基准测试中评估了类型分析的性能。

Conclusion: 通过类型安全的参数化流集成，为处理无界数据域的流式监控器提供了运行时安全保障，这对于复杂信息物理系统的安全监控至关重要。
